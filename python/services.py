...
class SpotifyService:
    ...
    def get_tracks_metadata(self, track_ids: str) -> Dict:
        ...
        for track_id in track_ids:
            if track_id["spotify_track_uri"] is not None and track_id["spotify_track_uri"].startswith("spotify:track:"):
...
class ElasticsearchService:
    ...
    def check_index(self):
        if self.client.indices.exists(index=self.index).body is False:
            self.client.indices.create(index=self.index, settings={
                "final_pipeline": "spotify"
            }, mappings={
