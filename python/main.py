...
def process_history_file(file_path: str, spotify_svc: SpotifyService, es_svc: ElasticsearchService):
    ...
    with Progress(
        ...
        task = progress.add_task(
            "[cyan]Processing tracks...", total=len(history))
        ...
        for i in range(0, total_entries, batch_size):
            ...
            for entry in entries_batch:
                try:
                    ...
                    # anything else yet.
                    if entry["spotify_track_uri"] is not None and entry["spotify_track_uri"].startswith("spotify:track:"):
                        track_id = entry["spotify_track_uri"].replace("spotify:track:", "")
                        metadata = metadata_batch.get(track_id, None)
                        played_at = try_parsing_date(entry["ts"])
                        if metadata is not None:
                            documents.append(
                                SpotifyTrack(
                                    id=str(int((played_at - datetime(1970, 1, 1)).total_seconds())) + "_" + entry["master_metadata_album_artist_name"],
                                    artist=[artist['name'] for artist in metadata['artists']],
                                    album=metadata["album"]["name"],
                                    country=entry["conn_country"],
                                    duration=metadata["duration_ms"],
                                    explicit=metadata["explicit"],
                                    listened_to_pct=entry["ms_played"]/metadata["duration_ms"] if metadata["duration_ms"] > 0 else None,
